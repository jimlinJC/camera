...
class Stereo:
    ...
    def stereo_bm(self):
        ...
        while True:
            ...
            Left_nice = cv.remap(
                frameL, Left_Stereo_Map[0], Left_Stereo_Map[1], cv.INTER_LANCZOS4, cv.BORDER_CONSTANT, 0)
            Right_nice = cv.remap(
                frameR, Right_Stereo_Map[0], Right_Stereo_Map[1], cv.INTER_LANCZOS4, cv.BORDER_CONSTANT, 0)
            ...
            stereo = cv.StereoBM_create(numDisparities=16*numDisparities, blockSize=SADWindowSize)
            stereo.setPreFilterCap(filterCap)
            stereo.setPreFilterSize(filterSize)
            disparity = stereo.compute(grayL, grayR)
            disparity.astype(np.float32)/16
            disp = cv.normalize(disparity, disparity, alpha=255,
                                beta=1, norm_type=cv.NORM_MINMAX, dtype=cv.CV_8UC1)
            cv.imshow("depth", disp)
            
            # cv.imshow("depth", disparity)
            # threeD = cv.reprojectImageTo3D(
            #     disparity.astype(np.float32)/16., config.Q)
            # print(threeD[0])
